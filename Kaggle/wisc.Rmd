...
In order to find the best number of trees to use for the prediction for the test data, we can use `gbm.perf` function. This
function returns the optimal number of trees for prediction.
...
`area_worst`.
We were able to predict with accuracy the malignant and benign tumors based on the three models used, `Random Forest`,
`Gradient Boosting Machine (GBM)` and `XGBoost`. For the `GBM` and `XGBoost` models we were also using cross validation.
The best prediction was obtained using `GBM` model.
